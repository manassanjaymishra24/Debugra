    ...
    <div style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <p style={{ fontSize: '0.8rem', color: '#64748b', textAlign: 'center', marginBottom: '24px' }}>
          {forgotMode ? 'Enter your email to receive a reset link' : !isLogin ? 'Sign up to save code & collaborate' : 'Sign in to access saved code'}
        </p>
        ...
        {forgotMode ? (
          ...
          <form onSubmit={handleForgotPassword}>
            <input value={resetEmail} onChange={(e) => setResetEmail(e.target.value)} placeholder="Email address"
              type="email" required style={{ ...inputStyle, marginBottom: '16px' }} />
            <button type="submit" disabled={loading} style={{
              width: '100%', padding: '10px', borderRadius: '10px', fontSize: '0.85rem', fontWeight: 600,
              background: '#4ec9b0', color: '#09090b', border: 'none', cursor: 'pointer', boxSizing: 'border-box'
            }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {isLogin && (
                <p style={{ textAlign: 'right', marginBottom: '12px' }}>
                  <button type="button" onClick={() => { setForgotMode(true); setResetEmail(email); }} style={{
                    background: 'none', border: 'none', color: '#4ec9b0', cursor: 'pointer',
