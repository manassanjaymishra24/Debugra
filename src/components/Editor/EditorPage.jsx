...
export default function EditorPage({ user }) {
  ...
  return (
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column' }}>
          ...
          {roomId && (
            <>
              ...
              <span className="topbar-title text-success d-none d-sm-inline">
                ✦ Room: {roomId} <span className="d-none d-lg-inline">({activeUsers.length} online)</span>
              </span>
              <button className="topbar-link ms-2" onClick={() => { navigator.clipboard.writeText(roomId); toast.success('Copied!'); }}>
      ...
      {/* ===== TOOLBAR ===== */}
      <div className="toolbar px-2 py-1 overflow-x-auto">
        <div className="toolbar-left d-flex align-items-center gap-2">
          <select className="form-select form-select-sm lang-select" value={language} onChange={handleLanguageChange} disabled={isReadOnly}>
            {Object.entries(LANGUAGES).map(([key, lang]) => (
              <option key={key} value={key}>{lang.name}</option>
            ))}
          </select>
          <div className="font-size-ctrl d-none d-sm-flex align-items-center gap-1">
            <button className="btn btn-dark btn-xs" onClick={() => fontSize > 10 && setFontSize(f => f - 1)}>−</button>
            <span className="small">{fontSize}px</span>
            <button className="btn btn-dark btn-xs" onClick={() => fontSize < 28 && setFontSize(f => f + 1)}>+</button>
          </div>
        </div>
        <div className="toolbar-right d-flex align-items-center gap-2">
          <div className="d-none d-md-flex align-items-center gap-2">
            <button className="ai-btn btn btn-sm" onClick={handleTests} disabled={isAILoading || isReadOnly}>Tests</button>
            <button className="ai-btn btn btn-sm" onClick={handleVisualize} disabled={isAILoading}>
              <svg className="me-1" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>
              Visualize
            </button>
            <button className="ai-btn btn btn-sm" onClick={handleExplain} disabled={isAILoading}>
              <svg className="me-1" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>
              Explain
            </button>
          </div>
          <button className="ai-btn fix btn btn-sm" onClick={handleFix} disabled={isAILoading || isReadOnly}>
            <svg className="me-1" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>
            Fix
          </button>
          <div className="d-flex align-items-center gap-1">
            <button className="toolbar-icon-btn btn btn-sm" onClick={handleDownload} title="Download code file">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
            </button>
            <button className="toolbar-icon-btn btn btn-sm" onClick={handleSave} title="Save to cloud" disabled={isReadOnly}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>
            </button>
            {user && (
              <button className="toolbar-icon-btn btn btn-sm" onClick={() => setShowHistory(!showHistory)} title="Saved code history" style={showHistory ? { background: 'var(--bg-active)', color: 'var(--accent)' } : {}}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
              </button>
            )}
          </div>
          <span className="kbd-hint d-none d-lg-inline">Ctrl+Enter</span>
          <button className="clear-btn btn btn-sm d-none d-sm-block" onClick={handleClear} disabled={isReadOnly}>Clear</button>
          <button className="run-btn btn btn-success btn-sm d-none d-sm-flex align-items-center" onClick={handleRun} disabled={isRunning}>
            {isRunning ? (
              <><span className="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true" /> Running...</>
            ) : (
              <><svg className="me-1" width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg> Run</>
            )}
          </button>
        </div>
      </div>
